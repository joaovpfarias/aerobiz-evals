#!/usr/bin/env python3
"""
ETAPA 1a-PartidaLonga: Avanca ~20 trimestres com end_turn, abre rotas, e captura
o estado do ranking regional em 2005.

OBJETIVO: gerar savestate com RegionalRankings ja com dados (caixas coloridas).
ENTREGA: states/eval_2005_rankings.state + screenshots dos relatorios.
"""
import sys
...
LOG_DIR = Path(__file__).parent.parent / "logs" / "advance_game"
...
def log_msg(msg):
    ...
    line = f"[{ts}] {msg}"
    print(line)
    (LOG_DIR / "run.log").open("a").write(line + "\n")

def main():
    log_msg("=" * 70)
    log_msg("INICIANDO ETAPA 1a-PartidaLonga")
    log_msg(f"Input:  {EVAL_STATE}")
    ...
    # === LOAD SAVESTATE ===
    log_msg(f"Carregando savestate {EVAL_STATE.name}...")
    if not EVAL_STATE.exists():
        log_msg(f"ERRO: savestate nao existe: {EVAL_STATE}")
        return False
    ...
    log_msg(f"Estado inicial: {date_label(quarter_start)} (trimestre {quarter_start}), Cash={cash_start}K")

    # === AVANCA 20 TRIMESTRES COM END_TURN ===
    target_quarter = quarter_start + 20
    ...
    while True:
        ...
        log_msg(f"  Turno {turns_executed + 1}: {date_label(quarter_now)} -> ?")
        ok, detail = game.end_turn()
        if ok:
            quarter_after = read_quarter_index(bridge)
            cash_after = read_cash_k(bridge)
            log_msg(f"    OK -> {date_label(quarter_after)}, Cash={cash_after}K: {detail}")
            turns_executed += 1
        else:
            log_msg(f"    FALHA: {detail}")
            turns_failed += 1
            if turns_failed > 3:
                log_msg("ABORTANDO: 3 falhas consecutivas em end_turn")
                return False

        time.sleep(0.5)  # Pequeno delay entre turnos

    log_msg(f"\nResumo: {turns_executed} turnos executados, {turns_failed} falhas")

    # === ABRE 2-3 ROTAS PARA GERAR RANKING ===
    log_msg("\nAbrindo rotas para gerar dados de ranking...")

    # Cidades estrategicas em regioes diferentes para ranking mais completo
    routes_to_open = [
        {"to": "London"},      # Europa
        {"to": "Tokyo"},       # SE Asia
        {"to": "Sydney"},      # Oceania
    ...
    quarterly_path = game.shot("quarterly_report_2005")
    log_msg(f"  Capturado: Quarterly Report -> {quarterly_path}")
    ...
    if on_quarterly_report_img(img):
        ...
        rankings_path = game.shot("regional_rankings_2005")
        log_msg(f"  Capturado: Regional Rankings -> {rankings_path}")
        ...
        if on_regional_rankings_img(img_rankings):
            ...
            # Recortes das 7 regioes (do mundo.py)
            regions = {
                "Europe": (24, 40, 88, 68),
            ...
            for region, (x0, y0, x1, y1) in regions.items():
                # Conta pixels coloridos (nao pretos)
                colored = 0
                total = (x1 - x0) * (y1 - y0)
                for x in range(x0, x1):
                    for y in range(y0, y1):
                        r, g, b = px[x, y]
                        # Se nao e preto (0,0,0), conta como cor
                        if (r, g, b) != (0, 0, 0):
                            colored += 1

                frac = colored / total if total > 0 else 0
                log_msg(f"      {region:12} -> {colored:4}/{total:4} = {frac:.1%} colorido")

                if colored > 0:
                    regions_with_data.append(region)

            if regions_with_data:
                log_msg(f"    ACHADO: {len(regions_with_data)} regioes com cores: {regions_with_data}")
            else:
                log_msg(f"    ACHADO: todas as caixas ainda estao pretas (sem dados)")
        else:
    ...
    if OUTPUT_STATE.exists():
        log_msg(f"SUCESSO: Savestate salvo -> {OUTPUT_STATE}")
    else:
        log_msg(f"ERRO: Savestate nao foi criado: {OUTPUT_STATE}")
        return False
    ...
    summary = {
        "start": {
            ...
            "date": date_label(quarter_start),
            "cash": cash_start,
        },
        "end": {
            "quarter": quarter_final,
            "date": date_label(quarter_final),
            "cash": cash_final,
        },
        "turns_executed": turns_executed,
        "turns_failed": turns_failed,
        "routes_opened": len(routes_to_open),
    ...
    log_msg(f"Trimestres avancados: {summary['end']['quarter'] - summary['start']['quarter']}")
    log_msg(f"Turnos executados: {summary['turns_executed']}")
    log_msg(f"Turnos falhados: {summary['turns_failed']}")
    log_msg(f"Rotas abertas: {summary['routes_opened']}")
    if summary['regions_with_data']:
        log_msg(f"Regioes com ranking: {summary['regions_with_data']}")
    log_msg(f"Savestate output: {OUTPUT_STATE}")
    log_msg("=" * 70)

    # Salva summary
    summary_file = LOG_DIR / "summary.json"
    summary_file.write_text(json.dumps(summary, indent=2))
    log_msg(f"Summary salvo: {summary_file}")
